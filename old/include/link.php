...
function addLink() {
	...
	if($_POST['send']){
		...
		if(count($errors) > 0){
			...
			$mydb->query($sql);
			if($mydb->result())
				echo info("You have successfully added <b>" . $name . "</b> as your link!") . "<br /><a href='?page=allMaps'>Back</a>";
		...
		<form method="post" action="<?php $PHP_SELF; ?>">
			...
			<table>
				<tr>
					...
					<td><input type="text" name="url" class="field" id="fieldUrl" /> <input type="button" class="button" id="driver" value="Get Title" />
					</td>
				</tr>
                <tr>
					<td>Title:
					</td>
					<td><input type="text" name="name" class="field" id="fieldTitle" /> 
					</td>
				</tr>
				<tr>
            ...
            <p class="pSubtitles">*Link title should be unique, so you can have more of a transperent view...</p>
		<?php
	}
}
...
function vsiLinki($user_id, $option, $mapId) {
	...
	if($mydb->recno() == 0) {
        ...
        <?php
            ...
            ?>
		<tr>
            <td class="tdLink<?php echo $i ?>">Either you have no links yet or you have to choose those links that you want to have on your home page <a href="?page=allMaps">here</a>!</td>
        </tr>
        <?php
        }
	} 
	...
	if($mydb1->recno() == 1) {
		...
		<?php
            ...
            if($count == 1) {
                ?>
                <tr>
                <td class="tdLinkHome" title="<?php echo $vrstica["url"] ?>"  onclick="window.open('<?php echo $vrstica["url"] ?>', '_blank');">
                    <?php
                    if($status == 2) {
...
function vsiLinkiManage($user_id, $mapId) {
	...
	if($mydb->recno() == 0) {
		...
		<tr>
            <td class="tdLink<?php echo $i ?>"></td>
            <td class="tdLink<?php echo $i ?>">No unmapped links!</td>
            <td class="tdLink<?php echo $i ?>"></td>
            <td class="tdLink<?php echo $i ?>"></td>
            <td class="tdLink<?php echo $i ?>"></td>
            <td class="tdLink<?php echo $i ?>"></td>
...
function enLinkManage($id, $i, $userId) {
	...
	if($mydb1->recno()) {
        ...
        <tr class="tdLink<?php echo $i ?>">
            ...
            <td>
                <input type="checkbox" name="homePage" <?php if($vrstica["status"] == 3) echo "checked"; ?> onchange="toHomePage(this, <?php echo $id ?>)" />
            </td>
            </td>
...
function sendLink() {
    ...
    if($_POST['send']){
		...
		if(count($errors) > 0){
			...
			$sql = "INSERT INTO links (url, domain, rating, user_id, name, submitter_id, status, map_id) VALUES ('" . $url . "','" . $domain . "', 0,'" . $recId["id"] . "','" . $name . "', '" . $_SESSION["portal_id"] . "', 2, 0)";
			$mydb->query($sql);
			if($mydb->result())
